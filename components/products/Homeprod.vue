<template>
  <div>
    <div class="relative h-fit">
      <etiquette
        class="absolute top-2 sm:right-4 right-2 z-1"
  ...
  methods: {
    ...
    manage_cart() {
      ...
      if (this.cookies.checkCookie('cart')) {
        ...
        if (!cart.some((el) => el.prod === this.prod.id)) {
          cart.push({ prod: this.prod.id, qty: +this.nb })
          this.cookies.setCookie('cart', cart, 365)
        } else {
          cart.forEach((el) => {
            if (el.prod === this.prod.id) el.qty += +this.nb
          })
          this.cookies.setCookie('cart', cart, 365)
        }
      } else {
